...
var
  ...
  n:  Integer;
  // Переменные игроков
  Player1Pos : Array[0..13,0..22] of Integer; // Вариант героя (морлок там гном или скелет)
  Player1Lvl : Integer; // Level бота
  Player1Zdor : Integer;
  Player1Mana : Integer;
  Player1Exp : Integer;      // Текуший опыт
  Player1ExpLvlUp : Integer; // Необходимый опыт до следующего уровня
  Player1Power : Integer;
  Player1Lovk : Integer;
  Player1Ydacha : Integer;
  Player1Zaklin : Array[0..15] of Integer; // Позиция и вариант заклинаний
  Player1Invent : array[0..15] of Integer; // Позиция и тип шмота
  Player1Race : Integer; // Раса (0 - Люди, 1 - Эльфы, 2 - Орки, 3 - Нежить, 4 - Монстры, 5 - Мирные)

  Player2Pos : Array[0..13,0..22] of Integer; // Вариант героя (морлок там гном или скелет)
  Player2Lvl : Integer; // Level бота
  Player2Zdor : Integer;
  Player2Mana : Integer;
  Player2Exp : Integer;      // Текуший опыт
  Player2ExpLvlUp : Integer; // Необходимый опыт до следующего уровня
  Player2Power : Integer;
  Player2Lovk : Integer;
  Player2Ydacha : Integer;
  Player2Zaklin : Array[0..15] of Integer; // Позиция и вариант заклинаний
  Player2Invent : array[0..15] of Integer; // Позиция и тип шмота
  Player2Race : Integer; // Раса (0 - Люди, 1 - Эльфы, 2 - Орки, 3 - Нежить, 4 - Монстры, 5 - Мирные)

  Player3Pos : Array[0..13,0..22] of Integer; // Вариант героя (морлок там гном или скелет)
  Player3Lvl : Integer; // Level бота
  Player3Zdor : Integer;
  Player3Mana : Integer;
  Player3Exp : Integer;      // Текуший опыт
  Player3ExpLvlUp : Integer; // Необходимый опыт до следующего уровня
  Player3Power : Integer;
  Player3Lovk : Integer;
  Player3Ydacha : Integer;
  Player3Zaklin : Array[0..15] of Integer; // Позиция и вариант заклинаний
  Player3Invent : array[0..15] of Integer; // Позиция и тип шмота
  Player3Race : Integer; // Раса (0 - Люди, 1 - Эльфы, 2 - Орки, 3 - Нежить, 4 - Монстры, 5 - Мирные)

  Player4Pos : Array[0..13,0..22] of Integer; // Вариант героя (морлок там гном или скелет)
  Player4Lvl : Integer; // Level бота
  Player4Zdor : Integer;
  Player4Mana : Integer;
  Player4Exp : Integer;      // Текуший опыт
  Player4ExpLvlUp : Integer; // Необходимый опыт до следующего уровня
  Player4Power : Integer;
  Player4Lovk : Integer;
  Player4Ydacha : Integer;
  Player4Zaklin : Array[0..15] of Integer; // Позиция и вариант заклинаний
  Player4Invent : array[0..15] of Integer; // Позиция и тип шмота
  Player4Race : Integer; // Раса (0 - Люди, 1 - Эльфы, 2 - Орки, 3 - Нежить, 4 - Монстры, 5 - Мирные)

  // Переменные ботов   Первое измерение массива - номер бота
  botPos : Array[0..19,0..13,0..22] of Integer; // Вариант бота (морлок там гном или скелет)
  botLvl : array[0..19] of Integer; // Level бота
  botZdor : array[0..19] of Integer;
  botMana : array[0..19] of Integer;
  botExp : array[0..19] of Integer;      // Текуший опыт
  botExpLvlUp : array[0..19] of Integer; // Необходимый опыт до следующего уровня
  botPower : array[0..19] of Integer;
  botLovk : array[0..19] of Integer;
  botYdacha : array[0..19] of Integer;
  botZaklin : Array[0..19,0..15] of Integer; // Позиция и вариант заклинаний
  botInvent : array[0..19,0..15] of Integer; // Позиция и тип шмота
  botProdaet : array[0..19] of Integer; // Если = 1 то у него можно покупать инвентарь
  botVrag : array[0..19] of Integer; // Если = 1 то он враждебен к тебе
  botRace : array[0..19] of Integer; // Раса (0 - Люди, 1 - Эльфы, 2 - Орки, 3 - Нежить, 4 - Монстры, 5 - Мирные)
...
begin
  ...
  img0m.Picture.LoadFromFile('.\Images\city_32.jpg'); ish[0,22]:=24; // Город
  ...
  img7b.Picture.LoadFromFile('.\Images\market_32.jpg'); ish[7,11]:=8; // рынок
  ...
  imgcc.Picture.LoadFromFile('.\Images\taverna_32.jpg'); ish[12,12]:=25; // таверна
  ...
  imgd0.Picture.LoadFromFile('.\Images\portal_32.jpg'); ish[13,0]:= 26; // Портал
...
// 172 - Женщина-эльф 7 ур
   ...
   sas := Random(6);      // Здесь псевдослучайным образом генерируется карта
   case sas of
   1:
     begin
...
if ish[0,0]= 8 then
begin
  mmo1.Text:='Объект: Рынок';
end;
end;

procedure TForm1.img0mClick(Sender: TObject);
begin
mmo1.Text:='Объект: Городские ворота' +#13#10 +#13#10;
mmo1.Text:=mmo1.Text + 'Из них обычно выходят люди и эльфы в поисках приключений. Бывает иногда крестьяне выйдут, странники или сбежавшые из тюрьмы орки.'+#13#10 +#13#10;
mmo1.Text:=mmo1.Text + 'Если раса Вашего героя Человек или Эльф, то мобы, выходящий из городских ворот дружественны к Вам не зависимо от времени суток, до тех пор пока Вы на них не нападёте';
...
procedure TForm1.btn4Click(Sender: TObject);  // Конец Хода
var
  newBotGeneration : integer;
  newBotPos : Array[0..19,0..13,0..22] of Integer; // Вариант бота (морлок там гном или скелет)
  newBotLvl : array[0..19] of Integer; // Level бота
  newBotZdor : array[0..19] of Integer;
  newBotMana : array[0..19] of Integer;
  newBotExp : array[0..19] of Integer;      // Текуший опыт
  newBotExpLvlUp : array[0..19] of Integer; // Необходимый опыт до следующего уровня
  newBotPower : array[0..19] of Integer;
  newBotLovk : array[0..19] of Integer;
  newBotYdacha : array[0..19] of Integer;
  newBotZaklin : Array[0..19,0..15] of Integer; // Позиция и вариант заклинаний
  newBotInvent : array[0..19,0..15] of Integer; // Позиция и тип шмота
  newBotProdaet : array[0..19] of Integer; // Если = 1 то у него можно покупать инвентарь
  newBotVrag : array[0..19] of Integer; // Если = 1 то он враждебен к тебе
  botRace : array[0..19] of Integer; // Раса (0 - Люди, 1 - Эльфы, 2 - Орки, 3 - Нежить, 4 - Монстры, 5 - Мирные)
  xBot : Integer;
  yBot : Integer;
  botaNet : integer;
...
begin
  ...
  begin
   ...
   begin
     ...
     if (newBotPos[nomerBota,yBot, xBot] = 0) and (botaNet = 308) then // Именно тут мы и создаём бота
       begin
         newBotPos[nomerBota,yBot, xBot] = newBotGeneration;
         if newBotPos[nomerBota,yBot, xBot] = 71 then   // Это эльф 1 ур
         begin
           botLvl[nomerBota] := 1; // Level бота
           botZdor[nomerBota] := 50;
           botMana[nomerBota] := 30;
           botExp[nomerBota] := 0;     // Текуший опыт
           botExpLvlUp[nomerBota] := 30; // Необходимый опыт до следующего уровня
           botPower[nomerBota] := 5;
           botLovk[nomerBota] := 3;
           botYdacha[nomerBota] := 1;
           botZaklin:[nomerBota,0]:=6;// Огненная сфера (6)
           //newBotInvent : array[0..19,0..15] of Integer; // Позиция и тип шмота
           botProdaet : array[0..19] of Integer; // Если = 1 то у него можно покупать инвентарь
           if (Player1Race = 2) or (Player1Race = 3) then botVrag[nomerBota] := 1;
           if (Player1Race = 0) or (Player1Race = 1) then botVrag[nomerBota] := 0;
           // Если = 1 то он враждебен к тебе
           botRace[nomerBota]:=1; // Раса (0 - Люди, 1 - Эльфы, 2 - Орки, 3 - Нежить, 4 - Монстры, 5 - Мирные)
         end;
         
         if newBotPos[nomerBota,yBot, xBot] = 71 then   // Это эльф 2 ур
         begin
           botLvl[nomerBota] := 1; // Level бота
           botZdor[nomerBota] := 50;
           botMana[nomerBota] := 30;
